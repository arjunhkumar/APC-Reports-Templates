...
for the class object containing the field.  
...
from a high number of comparisons similar to the native {\em String} type. Factors as such may be
determined by analyzing the source code statically in some cases, and in others may require 
...
can help to discover more impactful factors and access patterns to be considered for this 
optimization. 
...
order to selectively decide potential candidates for object flattening in the JVM.
 We also intend to expand the optimizations 
possible in JVM in the scope of 
value types. 
...
global VM datastructure. During the class initialization phase as described in section 3.1 we match the class signature of the
...
class cache initialization similar to native implementation described in section 3.1.
